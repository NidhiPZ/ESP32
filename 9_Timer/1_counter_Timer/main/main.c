...
#define TIMER_DIVIDER  16000  // Hardware timer clock divider (80 MHz / 80 = 1 MHz)
...
// #define TIMER_SCALE   (80000000 / TIMER_DIVIDER)  // Convert counter value to seconds
// #define TIMER_MAX_COUNT (200)  // Maximum counter value before triggering the interrupt
// #define LED_PIN GPIO_NUM_2  // Define LED pin

// // Define a speed factor for controlling increment speed (higher means slower)
// #define TIMER_INCREMENT_SPEED 2  // Set to 1 for normal speed, >1 for slower, <1 for faster
...
//         ESP_LOGI(TAG, "Counter value: %.f", time_elapsed);

//         // Check if the timer has reached 200 using the queue
//         if (xQueueReceive(timer_queue, &evt, 0)) {
//             if (evt.flag == 1) {
//                 ESP_LOGI(TAG, "Counter reached 2 seconds, LED turned OFF!");
