...
esp_err_t lps22hb_who_am_i(uint8_t device_addr)
{
    ...
    esp_err_t ret = i2c_read_bytes(device_addr, WHO_AM_I_REG, &who_am_i_reg, 1);
    
    if (ret == ESP_OK && who_am_i_reg == EXPECTED_WHO_AM_I)
    {
        ESP_LOGI(TAG, "WHO_AM_I matched: 0x%02X", who_am_i_reg);
        return ESP_OK;  // ID matched
    }
    else
    {
        ESP_LOGE(TAG, "WHO_AM_I mismatch. Expected: 0x%02X, Got: 0x%02X", EXPECTED_WHO_AM_I, who_am_i_reg);
