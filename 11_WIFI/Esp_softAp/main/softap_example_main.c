    ...
    if (event_id == WIFI_EVENT_AP_STACONNECTED)
     {
        ...
        printf("station "MACSTR"  DEVICE JOIND\n", MAC2STR(event->mac));
    } 
    else if (event_id == WIFI_EVENT_AP_STADISCONNECTED) 
    {
        wifi_event_ap_stadisconnected_t* event = (wifi_event_ap_stadisconnected_t*) event_data;
        printf("station "MACSTR" DEVICE LEAVE\n", MAC2STR(event->mac));
    }
}
...
void wifi_init_softap(void)
{
    ...
    esp_netif_create_default_wifi_ap();  //sets up necessary data structs for wifi station interface
